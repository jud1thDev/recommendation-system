...
Collects user behavior events. Current client use is click-based.
...
| `type` | string | yes | Current click event type is `product_click`. |
...
| `product.productId` | string | yes for clicks | Clicked product ID. |
| `product.category` | string | yes for clicks | Clicked product category. |
| `product.tags` | string[] | yes for clicks | Clicked product tags. |
| `product.priceValue` | number | yes for clicks | Clicked product price in KRW. |
| `product.rank` | number | no | Rank at impression/click time, when known. |
...
```text
Browser click -> rec-api POST /v1/events -> Kafka topic rec-events -> event-worker -> Redis
```
